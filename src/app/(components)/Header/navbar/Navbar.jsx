              ...
              data?.res?.data?.menu?.items?.length > 0 && (
                data?.res?.data?.menu?.items?.map(mainItem => (
                  <li className={`group relative`} key={mainItem?.id}>
                    {
                      mainItem?.url === '#' ? (
                        <span className={`hover:text-gray-600 hover:fill-gray-500 text-md flex items-center justify-between cursor-not-allowed`}>
                          <span>
                            {
                              hiddenOnMd ? "" : <svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" className="mr-4 inline-block" viewBox="0 0 64 64">
                                <path
                        ...
                        <Link href={`/collections/${getLastUrlPart(mainItem?.url)}`}
                          className={`hover:text-gray-600 hover:fill-gray-500 text-md flex items-center justify-between nav-link`}>
                          <span>
                            {
                              hiddenOnMd ? "" : <svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" className="mr-4 inline-block" viewBox="0 0 64 64">
                                <path
                    ...
                    <ul className={`absolute left-0 top-full bg-white shadow-md rounded-md w-40 z-10 invisible opacity-0 transition-all duration-300 ease-in-out group-hover:opacity-100 group-hover:visible`}>
                      {
                        mainItem?.items?.map(subItem => (
                          <li key={subItem?.id}>
                            {
                              subItem?.url === '#' ? (
                                <span className={`block px-4 py-2 hover:bg-gray-100 cursor-not-allowed`}>
                                  {subItem?.title}
                                </span>
                              ) : (
                                <Link href={`/collections/${getLastUrlPart(subItem?.url)}`} className={`block px-4 py-2 hover:bg-gray-100`}>
                                  {subItem?.title}
                                </Link>
                              )
                            }
